...
@Component({
  selector: 'app-bandeja-jefe-departamento',
  templateUrl: './bandeja-jefe-departamento.component.html',
  styleUrls: ['./bandeja-jefe-departamento.component.scss']
})
export class BandejaJefeDepartamentoComponent implements OnInit {
  ...
  bandejaJefeUnidad = new MatTableDataSource();

  @ViewChild('MatPaginator2') set matPaginator2(mp2: MatPaginator) {
    this.bandejaJefeUnidad.paginator = mp2;
  }
  ...
  Approve() {
    ...
    this.alcanceService.ApproveJd(this.idAlcance).toPromise().then(res => {
      console.log(res);
      this.dialogo.show({
        icon: 'success',
        title: 'IFI-200',
  ...
  getCabinet() {
    this.alcanceService.getCabinetJd(976).toPromise().then(res => {
      console.log(res);
      const key = 'idAlcance';
      const unique = [...new Map(res.map(item => [item[key], item])).values()]
      console.log(unique);
      this.bandejaJefeUnidad.data = unique;
    })
  }

  getPresence() {
    this.alcanceService.getPresenceJd(973).toPromise().then(res => {
      console.log(res);
      const key = 'idAlcance';
      const unique = [...new Map(res.map(item => [item[key], item])).values()]
      console.log(unique);
  ...
  getPoint() {
    this.alcanceService.getPointJd(975).toPromise().then(res => {
      console.log(res);
      const key = 'idAlcance';
      const unique = [...new Map(res.map(item => [item[key], item])).values()]
      console.log(unique);
    ...
    switch (parseInt(this.selectedValue)) {
      case 1:
        this.alcanceService.getPresence(973).toPromise().then(res => {
          console.log(res);
          this.bandejaJefeUnidad.data = res;
        })
        break;
      case 2:
